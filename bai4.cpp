...
void LineSegmentTest :: testCase1() {
    Point A(2, 5), B(20, 35);
    LineSegment AB(A, B);
    AB.diChuyen(35, 51);
    cout << "Ket qua testCase1: ";
    AB.in();
    cout << endl;
}

void LineSegmentTest :: testCase2() {
    LineSegment CD;
    cout << "Nhap toa do cho CD: ";
    CD.nhap();
    cout << fixed << setprecision(2);
    cout << "Ket qua testCase2: |CD| = " << CD.doDai() << endl;
}

void LineSegmentTest :: testCase3() {
    int n;
    ...
    for (int i = 0; i < n; i++) {
        cout << "Nhap doan thu " << i + 1 << ": ";
        segments[i].nhap();
    }

    sort(segments.begin(), segments.end(), [](const LineSegment& a, const LineSegment& b) {
        return a.doDai() < b.doDai();
    });

    cout << "Danh sach doan thang theo chieu dai tang dan:\n";
